...
namespace RopeToolkit
{
    ...
    public class MyRope : MonoBehaviour
    {
        ...
        /// <param name="maxImpulseStrength">The max allowable impulse strength to use. If zero, no limit is applied.</param>
        ...
        public void SetPositionAt(int particleIndex, float3 position, float maxImpulseStrength)
        {
            ...
            {
                var particleMass = massMultipliers[particleIndex] * simulation.massPerMeter * _measurements.realCurveLength / _measurements.particleCount;
        ...
        protected MyRope InstantiateSplitRope(int minIdx, int maxIdx, string identifier)
        {
            ...
            var rope = Instantiate(gameObject, Vector3.zero, Quaternion.identity).GetComponent<MyRope>();
        ...
        public void SplitAt(int particleIndex, MyRope[] outNewRopes = null)
        {
        ...
        public void FixedUpdate()
        {
            ...
            // Nach der Simulation: Endpunkte explizit an Rigidbody-Positionen setzen, falls RopeConnection aktiv ist
            // (Optional: Hier prüfen, ob ein RopeConnection-Skript am jeweiligen Endpunkt existiert und Rigidbody zugewiesen ist)
            var startConnection = GetComponent<RopeConnection>();
            if (startConnection != null && startConnection.rigidbodySettings.body != null)
            {
                SetPositionAt(0, startConnection.rigidbodySettings.body.transform.position);
            }
            var endConnection = GetComponent<RopeConnection>(); // Beispiel: falls du ein separates Skript für das Seil-Ende hast
            if (endConnection != null && endConnection.rigidbodySettings.body != null)
            {
                SetPositionAt(positions.Length - 1, endConnection.rigidbodySettings.body.transform.position);
            }
        ...
        {
            // Test-Gizmo: immer eine rote Kugel an der Transform-Position
        ...
        private struct SimulateJob : IJob
        {
            ...
            private void ApplyStickConstraint(int idx0, int idx1)
            {
                ...
                if (w0 > 0.0f) w0 = 1.0f / w0;
                var w1 = massMultipliers[idx1];
                if (w1 > 0.0f) w1 = 1.0f / w1;
                var invSumW = w0 + w1;
                if (invSumW > 0.0f) invSumW = 1.0f / invSumW;

                if (stiffness >= 0.999f)
                {
                    // Exakte Korrektur: Kein Stretch möglich
                    float correction = dist - desiredSpacing;
                    positions[idx0] -= delta * (correction * w0 * invSumW);
                    positions[idx1] += delta * (correction * w1 * invSumW);
                }
                else
                {
                    // Standard: Weiche Korrektur
